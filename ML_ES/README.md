...
## RL-First Two-Stage Training

This repo now includes a minimal RL training path under `rlplace/` and `scripts/`:

1. Train a large graph policy with evolution strategies from scratch.
2. Distill a smaller student directly from the teacher's deterministic actions.

Both stages:

- write JSONL logs under `logs/`
- save resumable `.pt` checkpoints under `checkpoints/`
- use an adaptive learning-rate scheduler that raises LR when monitored benchmark cost improves and cuts it when benchmark cost regresses

Stage 1 optimizes the final benchmark directly:

- final average overlap dominates the score
- final normalized wirelength only matters as a secondary tie-breaker
- ES never uses a critic or cumulative rollout reward

Run the two stages with:

```bash
python3 -m scripts.train_teacher_es --config configs/teacher_es.json
python3 -m scripts.distill_student --config configs/student_distill.json
python3 -m scripts.eval_policy --config configs/eval_policy.json --tests 1:10
```

The default eval config points at the Stage 1 teacher checkpoint. After Stage 2 finishes,
switch `configs/eval_policy.json` to the distilled student checkpoint if you want to
measure the compressed policy instead.
